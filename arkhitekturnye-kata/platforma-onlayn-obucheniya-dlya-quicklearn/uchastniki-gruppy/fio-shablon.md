---
order: 1
title: ФИО Кольчик Константин Васильевич
---

## 1\. 📖 Описание функциональных границ микросервиса

### 1\.1 Диаграмма компонентов архитектуры

![](./fio-shablon.webp){width=1536px height=1024px}

### 1\.2 Описание микросервиса

**Rating Service** -- микросервис системы QuickFix, отвечающий за работу с оценками и отзывами о мастерах. Он выделен из основного монолита и взаимодействует с ним через REST API.

Микросервис выполняет следующие функции: создание оценки мастеру после завершения заявки, сохранение текстового отзыва, получение оценок и отзывов по конкретному мастеру, расчёт его среднего рейтинга. Функциональные границы сервиса ограничены только работой с рейтингами и отзывами. Он **не отвечает** за создание и обработку заявок, управление пользователями, проведение оплаты, поиск мастеров, работу с картами и отправку уведомлений -- эти функции остаются в основном приложении QuickFix. В исходной архитектуре именно заявки, пользователи, оплата и интеграции относятся к монолиту. \
Основные данные микросервиса: идентификатор заявки, идентификатор заказчика, идентификатор мастера, оценка, текст отзыва и дата создания. Отдельная база данных используется для хранения оценок и отзывов.

Также для сервиса действует бизнес-правило: **одну завершённую заявку можно оценить только один раз**.

## 2\. 🧩 Концептуальное проектирование API метода

{% table header="row" %}

---

*  {% colwidth=[156] %}

   Потребители

*  {% colwidth=[156] %}

   Цель

*  {% colwidth=[156] %}

   Задачи

*  {% colwidth=[192] %}

   Входные данные

*  {% colwidth=[239] %}

   Выходные данные

---

*  {% colwidth=[156] %}

   Front-end заказчика

*  {% colwidth=[156] %}

   Оценить работу мастера

*  {% colwidth=[156] %}

   Передать оценку и отзыв по завершённой заявке

*  {% colwidth=[192] %}

   Идентификатор заявки, идентификатор мастера, оценка, текст отзыва

*  {% colwidth=[239] %}

   Идентификатор оценки, результат сохранения

---

*  {% colwidth=[156] %}

   Front-end заказчика

*  {% colwidth=[156] %}

   Получить рейтинг мастера

*  {% colwidth=[156] %}

   Запросить текущий рейтинг мастера

*  {% colwidth=[192] %}

   Идентификатор мастера

*  {% colwidth=[239] %}

   Средний рейтинг, количество оценок

---

*  {% colwidth=[156] %}

   Front-end заказчика

*  {% colwidth=[156] %}

   Получить отзывы о мастере

*  {% colwidth=[156] %}

   Запросить отзывы по мастеру

*  {% colwidth=[192] %}

   Идентификатор мастера

*  {% colwidth=[239] %}

   Список оценок и отзывов

---

*  {% colwidth=[156] %}

   Front-end мастера

*  {% colwidth=[156] %}

   Просмотреть свой рейтинг

*  {% colwidth=[156] %}

   Запросить рейтинг и отзывы текущего мастера

*  {% colwidth=[192] %}

   Идентификатор мастера

*  {% colwidth=[239] %}

   Средний рейтинг, количество оценок, отзывы

{% /table %}

## 3\. 🤝 Swagger

[openapi:./_index-2.yaml:true]



### 